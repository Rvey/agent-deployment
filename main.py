...
from livekit.agents import AutoSubscribe, JobContext, WorkerOptions, WorkerType, cli, JobProcess
...
async def entrypoint(ctx: JobContext):
    ...
    eleven_tts=elevenlabs.tts.TTS(
        ...
        language=participant_attributes.get("language_tts"),
        streaming_latency=3,
    ...
    agent = VoicePipelineAgent(
        ...
        stt=openai.STT(model="whisper-1", language=participant_attributes.get("language_stt")),
        llm=openai.LLM(model="gpt-4o"),
        tts=eleven_tts,
...
if __name__ == "__main__":
    cli.run_app(WorkerOptions(entrypoint_fnc=entrypoint,prewarm_fnc=prewarm,worker_type=WorkerType.ROOM))
